...
export class Api {
    ...
    private async fetch(
        ...
        const targetStatus = this.expectedStatus === -1 ? this.getAutoMappedStatus(method) : this.expectedStatus;

        // Auto-check status if checkStatus is enabled and expectedStatus is set
        if (this.checkStatus && this.expectedStatus !== -1 && this.response.status() !== targetStatus) {
            throw new Error(
     ...
     */
    async getRequest(params?: any, headers?: Record<string, string>, expectedStatus?: number): Promise<this> {
     ...
     */
    async postRequest(data?: any, headers?: Record<string, string>, expectedStatus?: number): Promise<this> {
        if (expectedStatus !== undefined) this.expectedStatus = expectedStatus;
