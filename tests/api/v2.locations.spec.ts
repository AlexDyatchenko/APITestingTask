...
test.describe(
  ...
  () => {
    ...
    test.describe(
      ...
      () => {
        test("should return 200 with a valid Bearer token", async () => {
        ...
        test(
          ...
          async () => {
            ...
            await api.getRequest(
              PayloadBuilder.generateValidLocationParams(),
              undefined,
              -1,
            );
    ...
    test.describe("Validation & Schema", () => {
      ...
      test("should validate status codes for success", async () => {
        await api.getRequest({ locationStatuses: LOCATION_STATUSES.ACTIVE });
    ...
    test.describe(
      "Functional Test Cases - Success Path",
      {
        tag: [],
      },
      () => {
        ...
        test("should filter locations by status", async () => {
          const body = await api
            .getRequest({ locationStatuses: LOCATION_STATUSES.ACTIVE })
            .then(() => api.getBody());
        ...
        test("should filter locations by metro - Singapore", async () => {
          const body = await api.params({ metro: "Singapore" }).get();
          ...
          const actualLocationNames = body.data
            .map((loc: any) => loc.name)
            .sort();
          const expectedNames = [
            ...expectedSingaporeLocations.expectedLocationNames,
          ].sort();
          ...
          if (unexpectedLocations.length > 0) {
            console.log(
              `Note: Found ${unexpectedLocations.length} additional Singapore locations not in expected list:`,
              unexpectedLocations,
            );
          }
        });
        ...
        test("should filter by marketEnabled parameter", async () => {
          const body = await api
            .getRequest({ marketEnabled: true })
            .then(() => api.getBody());
          expect(body).toHaveDataArray();
        });

        test("should filter by mveVendor parameter - Aruba", async () => {
          const body = await api
            .getRequest({ mveVendor: MVE_VENDORS.ARUBA })
            .then(() => api.getBody());
          expect(body).toHaveDataArray();
        ...
        test("should filter by mveVendor parameter - Cisco", async () => {
          const body = await api
            .getRequest({ mveVendor: MVE_VENDORS.CISCO })
            .then(() => api.getBody());
          expect(body).toHaveDataArray();
        });

        test("should handle multiple locationStatuses values", async () => {
          ...
          if (body.data.length > 0) {
            body.data.forEach((location: any) => {
              expect([
                LOCATION_STATUSES.ACTIVE,
                LOCATION_STATUSES.DEPLOYMENT,
              ]).toContain(location.status);
            });
          }
        });

        test("should validate diversityZones object in response", async () => {
          const body = await api
            .getRequest({ locationStatuses: LOCATION_STATUSES.ACTIVE })
            .then(() => api.getBody());
          ...
          if (body.data.length > 0) {
            ...
            if (locationWithDiversity) {
              ...
              expect(typeof locationWithDiversity.diversityZones).toBe(
                "object",
              );
              ...
              const zoneTypes = Object.keys(
                locationWithDiversity.diversityZones,
              );
        ...
        test("should validate MVE sizes and details in response", async () => {
          const body = await api
            .getRequest({ mveVendor: MVE_VENDORS.ARUBA })
            .then(() => api.getBody());
        ...
        test("should validate latitude and longitude fields", async () => {
          const body = await api
            .getRequest({
              locationStatuses: LOCATION_STATUSES.ACTIVE,
              metro: "Singapore",
    ...
    test.describe(
      "Equivalence Partitioning - locationStatus",
      {
        tag: [],
      },
      () => {
        ...
        validStatuses.forEach((status) => {
          test(`should accept valid locationStatuses: ${status}`, async () => {
            const body = await api
              .getRequest({ locationStatuses: status })
              .then(() => api.getBody());
            expect(body).toHaveDataArray();
        ...
        test(
          ...
          async () => {
            await api.getRequest(
              { locationStatuses: "InvalidStatus123" },
              undefined,
              -1,
            );
    ...
    test.describe(
      "Equivalence Partitioning - mveVendor",
      {
        tag: [],
      },
      () => {
        ...
        validVendors.forEach((vendor) => {
          test(`should accept valid mveVendor: ${vendor}`, async () => {
            const body = await api
              .getRequest({ mveVendor: vendor })
              .then(() => api.getBody());
            expect(body).toHaveDataArray();
          });
        });
    ...
    test.describe(
      "Equivalence Partitioning - marketEnabled",
      {
        tag: [],
      },
      () => {
        test("should accept marketEnabled as true", async () => {
          const body = await api
            .getRequest({ marketEnabled: true })
            .then(() => api.getBody());
          expect(body).toHaveDataArray();
        ...
        test("should accept marketEnabled as false (default)", async () => {
          const body = await api
            .getRequest({ marketEnabled: false })
            .then(() => api.getBody());
          expect(body).toHaveDataArray();
        });
    ...
    test.describe(
      ...
      () => {
        test("should handle minimum boundary values", async () => {
          const body = await api
            .getRequest(PayloadBuilder.generateMinBoundaryParams())
            .then(() => api.getBody());
        ...
        test("should handle maximum boundary values", async () => {
          await api.getRequest(
            PayloadBuilder.generateMaxBoundaryParams(),
            undefined,
            -1,
          );
    ...
    test.describe(
      ...
      () => {
        test("should return 400 Bad Request for invalid parameters", async () => {
          await api.getRequest(
            PayloadBuilder.generateInvalidParams(),
            undefined,
            -1,
          );
        ...
        test("should handle invalid Accept header gracefully", async () => {
          await api.getRequest(
            {
              locationStatuses: LOCATION_STATUSES.ACTIVE,
            },
        ...
        test("should return appropriate error for non-existent endpoint", async () => {
          await api
            .path("/v2/locations/nonexistent123456")
            .getRequest(null, null, -1);
        ...
        test(
          ...
          async ({ request }) => {
            ...
            const promises = Array.from({ length: concurrentRequests }, () => {
              const mock = new MockAPIRequestContext();
              return new Api(mock as any, api.baseURL).getRequest(
                PayloadBuilder.generateValidLocationParams(),
              );
            });
    ...
    test.describe(
      ...
      () => {
        test("should handle dynamically generated query parameters", async () => {
        ...
        test("should generate unique test data on each run", async () => {
    ...
    test.describe("Additional Metro Filter Tests", () => {
      test(
        ...
        async () => {
          const body = await api.params({ metro: "London" }).get();
      ...
      test(
        ...
        async () => {
          const body = await api.params({ metro: "NonExistentMetro123" }).get();
